...
- **Six documents in the GitHub fixture read differently, each for a stated reason.** GitHub adds a "Details" summary where none is written. The HTML parser moves stray table text and a caption in front of the table. A style element the tag filter misses shows its text undecoded, and a used footnote moves to the foot of the page.
...
- **The reader follows GitHub's sanitiser, checked against GitHub itself.** GitHub unwraps every element outside its allowlist, so `sh<form></form>all` shows the banned word, and an `audio` element's text shows. A script that the tag filter misses, such as `<script/x>`, is removed to the end of the document. What GitHub's Markdown API returned for 290 documents is now a fixture, and the reader matches it on 284.
- **Footnotes read as GitHub shows them.** A footnote nothing refers to is left off the page, so it is no longer audited or counted as visible text. A footnote something refers to is read without its label.
- **One raw HTML block can hold several paragraphs.** Six `<p>` elements in one block were read as one paragraph six sentences long. A hidden element left open, such as a video around blank lines, now hides the blocks after it until it closes.
- **Two output style examples keep to their own rules.** Example 6 added a test assertion the "before" never made, so its list now names only the three areas. Example 12 left "token bucket algorithm" and "mutex" undefined, which Part 3 forbids, so it now defines both.
