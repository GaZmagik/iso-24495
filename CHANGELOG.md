...
- **Thirteen documents in the GitHub fixture read differently, each for a stated reason.** GitHub adds a "Details" summary where none is written. The HTML parser moves stray table text and a caption in front of the table. A style element the tag filter misses shows its text undecoded, and a used footnote moves to the foot of the page, which accounts for eight of them.
...
- **Two output style examples keep to their own rules.** Example 6 added a test assertion the "before" never made, so its list now names only the three areas. Example 12 left "token bucket algorithm" and "mutex" undefined, which Part 3 forbids, so it now defines both, and its five-sentence paragraph is now two.
