...
- The output style gains a **Reporting work** section, and the send-time check grows from four measures to six. Two external reviews judged 165 of this project's own replies against the four governing principles, and found failures the sentence and paragraph limits cannot see: a verdict reported without its findings, work called done while a gate was still open, options described unevenly so the reader is steered rather than informed, a rule contradicted two paragraphs after it was given, and grammar dropped in the name of brevity. Each is now a rule with a matching item in the send-time check.
