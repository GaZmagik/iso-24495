...
- **Seven documents in the GitHub fixture read differently, each for a stated reason.** GitHub adds a "Details" summary where none is written. The HTML parser moves stray table text and a caption in front of the table. A style element the tag filter misses shows its text undecoded, and a used footnote moves to the foot of the page.
...
- **Content the page hides is not read, and content it shows is.** A browser shows a video element's controls rather than its fallback text, and gives `rp` no box. An `rp` element also ends before an `rt` element, where its end tag may be left out. A script or style element is read, because GitHub's tag filter shows its tags and content as text.
- **The reader follows GitHub's sanitiser, checked against GitHub itself.** GitHub unwraps every element outside its allowlist, so `sh<form></form>all` shows the banned word, and an `audio` element's text shows. A script that the tag filter misses, such as `<script/x>`, is removed to the end of the document. What GitHub's Markdown API returned for 305 documents is now a fixture, and the reader matches it on 298. A separate small reader reads GitHub's side, so a mistake in the engine cannot cancel itself out.
- **Footnotes read as GitHub shows them.** A footnote nothing refers to is left off the page, so it is no longer audited or counted as visible text. A reference is read from the Markdown, so an escaped bracket, a character reference, a code span, a tag, a link destination or a raw HTML block refers to nothing. Everything inside an unused footnote goes, a heading included. A footnote is read without its label, and its later paragraphs, indented four columns, are part of it.
