...
- **Sixteen documents in the GitHub fixture read differently, each for a stated reason.** GitHub adds a "Details" summary where none is written. The HTML parser moves stray table text and a caption in front of the table. A style element the tag filter misses shows its text undecoded, and a used footnote moves to the foot of the page, which accounts for eleven of them.
