...
- Quotations stay outside the audit, and the distinction is deliberate. A list item is the writer's own prose. A quotation is someone else's words, or a demonstration of the writing a document warns against, and advising the writer repairs neither.
- `legalese`, `wordy-phrase` and `double-negative` ignore a word in quotes, backticks or emphasis, as `complex-word` has since it shipped. A skill teaching "never write shall" was told never to write it, and the advice "to, not in order to" was reported as wordy.
