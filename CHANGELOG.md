...
- **The text audit states that it supports English only.** Its word rules match English words, and its counting rules count words by spaces against English benchmarks. The README and the audit skill now say so, and say that the audit reads Markdown as written rather than interpreting raw HTML.
- **Continuous integration moves to a newer Bun release.** The workflows now pin Bun 1.4.2 instead of 1.3.14.
