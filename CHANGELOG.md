...
- **A script element's content is no longer read.** GitHub's sanitiser removes the element with its content, so `sh<script>x</script>all` shows the banned word, while the reader dropped the tags and read the content between. The reader now consumes a script element whole, across lines, and a browser gives `rp` no box, so its content goes too. A style, svg or iframe element keeps its text, because the published pipeline unwraps it, and measuring text the page hides is the safer error.
