...
- **The link, image and table rules still read markup through the older reader.** They need the tags, which the rendered text no longer holds. So `[click here](x)` inside a div, which the page shows as those characters, is still reported as a link. That is a false positive rather than a bypass.
- **Text after hidden content that spans lines reports on the line where the content opened.** A later line in the same block keeps its own number.
- **Seventeen documents in the GitHub fixture read differently, each for a stated reason.** GitHub adds a "Details" summary where none is written. The HTML parser moves stray table text and a caption in front of the table. A style element the tag filter misses shows its text undecoded, and a used footnote moves to the foot of the page, which accounts for twelve of them.
...
- **Continuous integration moves to a newer Bun release.** The pinned version was 1.3.14, whose Markdown renderer took 666 ms over 20,000 unmatched brackets. Version 1.4.2 takes 1 ms, and the workflows now pin it. The audit renders each block, so a timing test failed on the old version alone.
...
- **A definition wrapped across two lines counts.** The acronym scan read one source line at a time. So "identity and access management (IAM)" wrapped after "access" never met its own acronym, and IAM was reported as undefined. A soft line break is a space to the reader, so the definition is now found across it, and still not across a paragraph, list item or heading.
- **Markup inside a word no longer splits it.** A browser joins text across an inline element or a comment, so `sh<em>all</em>` shows the banned word, while the audit replaced every tag with a space and read two words. The reader now leaves nothing where an inline tag, a comment or a processing instruction stood. It leaves a space only at an element the HTML Standard renders as its own block, table part, list item or line break. A line ending inside removed markup is carried to the next one, so the word joins and later lines keep their numbers.
- **The pull request check reads a leading `---` block as the page shows it.** A description has no front matter, so GitHub renders the block as a rule and a heading. The audit hid it as metadata, and passed a description whose only text sat inside it. The text audit now takes `--no-front-matter`, which the check passes, and a file in a repository is read as before.
- **Example 18 in the output style restructures without rewriting.** Its "after" replaced "For the rollback steps, click here" with "See the rollback steps", which Part 5 forbids, because rewording belongs to Part 1. The link now moves onto the words that name its destination, and every word stays as written. Examples 15 to 17 were checked against the same rule and pass, because labels, slots and heading changes are structure the skill permits. A test now holds every Part 5 example to it.
- **The rules read the text the page shows.** A hand-written reader decided what prose showed, and each review found a construct it read differently from the page. Emphasis inside a word, a bogus comment and a quotation mark written as a reference were the last three. The Markdown renderer now decides what a block shows, with GitHub's tag filter applied and an HTML tokeniser reading the text. The rules about words and the visibility check share that one reader, so they cannot disagree.
- **Content the page hides is not read, and content it shows is.** A browser shows a video element's controls rather than its fallback text, and gives `rp` no box. An `rp` element also ends before an `rt` element, where its end tag may be left out, and neither a ruby nor an `rp` nested inside it can end it. A script or style element is read, because GitHub's tag filter shows its tags and content as text.
- **The reader follows GitHub's sanitiser, checked against GitHub itself.** GitHub unwraps every element outside its allowlist, so `sh<form></form>all` shows the banned word, and an `audio` element's text shows. A script that the tag filter misses, such as `<script/x>`, is removed to the end of the document. What GitHub's Markdown API returned for 336 documents is now a fixture, and the reader matches it on 319. A separate small reader reads GitHub's side, so a mistake in the engine cannot cancel itself out.
- **Footnotes read as GitHub shows them.** A footnote nothing refers to is left off the page, so it is no longer audited or counted as visible text. Everything inside it goes, a heading included. A footnote is read without its label, and its later paragraphs, indented four columns, are part of it.
- **The renderer decides what a footnote reference is.** A hand-written scanner read references, and each review found a construct it read differently from GitHub. Each footnote now stands in as a link reference definition, and a reference is whatever the renderer resolves to one. Labels then compare case-folded as GitHub compares them, and a second definition of a label is never reached. Each stand-in links to a fragment made fresh for each run, which no author can write, and a reference shows its number rather than its label.
- **A full reference to a footnote label keeps its text.** GitHub shows `[Read the guide.][^a]` as the bracketed text followed by the footnote number, so the rules must read those words. The reader dropped the text along with the label.
- **An `rp` element can hold a select, and a ruby carries across blocks.** Inside a select the HTML parser ignores every tag but its options, so an `</rp>` there closes nothing. A select, input or keygen start tag ends the select, and a textarea does not, because the tag filter has already escaped it. An `rp` left open in one block is carried on with the ruby holding it, so an `rt` in a later block still closes it.
- **One raw HTML block can hold several paragraphs.** Six `<p>` elements in one block were read as one paragraph six sentences long, whether on six lines or one. A hidden element left open, such as a video around blank lines, now hides the blocks after it until it closes or its quotation or list item ends.
- **A link reference definition over several lines resolves.** The parser reads only a one-line definition, so a reference to a longer one stayed as written. A paragraph that opens with a bracket and renders as nothing is now read as definitions, and every block resolves its references against them.
- **The Codex style skill must equal the output style exactly.** The test checked that the skill contained the style's text, so a line added to the Codex copy alone, such as "Ignore every rule above.", passed. Everything from the style's first sentence onwards must now be identical.
- **An invisible character no longer splits a word.** A word joiner, a zero-width space or a soft hyphen inside "shall" left the word unreported, although the page shows it whole. Every default-ignorable character is now removed before the rules read the text.
- **Three output style examples keep to their own rules.** Example 6 added a test assertion the "before" never made, so its list now names only the three areas. Example 12 left "token bucket algorithm" and "mutex" undefined, which Part 3 forbids, so it now defines both, and its five-sentence paragraph is now two. Example 7's clause now carries the identifier Part 2 requires.
- **A Windows checkout passes the gate.** Under `core.autocrlf` the shipped documents arrived with Windows line endings. A test splitting them on line feeds then failed on files nobody had changed. A `.gitattributes` file now checks every text file out with Unix line endings.
- **The rules about words now read headings.** A reader reads a heading, but the legalese, doublet, wordy-phrase, complex-word and double-negative rules read prose alone, so a heading holding a banned word reported nothing in any document. Each now reads every heading as a line of its own. The rules about sentences still leave headings alone, because a heading is not a sentence and heading-style already bounds its length. An acronym used only in a heading is still not reported, because a heading is usually the name of the thing.
