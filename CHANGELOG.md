...
- **Eleven documents in the GitHub fixture read differently, each for a stated reason.** GitHub adds a "Details" summary where none is written. The HTML parser moves stray table text and a caption in front of the table. A style element the tag filter misses shows its text undecoded, and a used footnote moves to the foot of the page, which accounts for six of them.
...
- **A footnote reference is read from the Markdown.** An escaped one, one written as a character reference, and one inside code, a tag, a comment or a link destination all refer to nothing. So does one in a raw HTML block. Labels compare case-folded, and a second definition of a label shows nothing.
