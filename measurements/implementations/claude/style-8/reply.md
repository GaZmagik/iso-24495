Here is `evaluate.ts`. It is a recursive-descent parser over the raw string, tracking a single scan index so every error carries the exact position.
...
**Design notes on two points the spec pins down:**

- **`-2^2` is -4.** `parseUnary` delegates to `parsePower`, so the negation wraps the whole power. The right operand of `^` calls back into `parseUnary`, which gives both right associativity (`2^3^2` is 512) and support for `2^-3`.
- **Number shapes.** `parseNumber` counts digits across both sides of the decimal point, so `.5` and `10.` parse while a lone `.` raises a `SyntaxError`.

**Status:** written and hand-traced against all five error cases in the spec, plus `2^3^2`, `-2^2` and the division-by-zero path. I did not compile it or run tests, because you asked for output only and no files were written. `tsc` and a test run are the checks still open.
