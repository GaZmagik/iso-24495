The command needed approval, so I did not run it. Here is `evaluate.ts`, hand-checked against every example in the spec but not executed.
...
**Status:** written and traced by hand; not compiled and not run, because the Node check I attempted required approval.

Traces of the five error cases in the spec:

| Input | Position | Message |
| --- | --- | --- |
| `2 +` | 3 | `Expected a number or "(" but found end of input at position 3` |
| `(1 + 2` | 6 | `Expected ")" but found end of input at position 6` |
| `1 + )` | 4 | `Expected a number or "(" but found ")" at position 4` |
| `` (empty) | 0 | `Expected a number or "(" but found end of input at position 0` |
| `1 $ 2` | 2 | `Expected end of input but found "$" at position 2` |

Two details worth knowing. Trailing junk such as `1 + 2)` is caught by `expectEnd`, which reports position 5. A malformed number such as `1..2` parses `1.` and then fails at position 2, because `expectEnd` finds the second dot.
