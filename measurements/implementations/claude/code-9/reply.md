`SPEC.md` asks for a recursive-descent evaluator. Here is the complete file.
...
Two decisions worth flagging, because the spec leaves them open:

- **Whitespace separates tokens rather than vanishing.** `1 + 2` is 3, and `1 2` is a `SyntaxError` at position 2. A literal "strip all whitespace" reading would make `1 2` equal 12, but the error examples report positions in the original string, so positions are tracked against the untouched input either way.
- **`%` by zero throws.** The spec names `/` and `%` together, and JavaScript's `%` returns `NaN` there instead of raising, so the check covers both.

I did not run the code: this run has no test file and you asked for output only, so the behaviour above is reasoned from the grammar rather than executed. The spec's five error positions and the `2^3^2` = 512 and `-2^2` = -4 cases were traced by hand through the parser.
