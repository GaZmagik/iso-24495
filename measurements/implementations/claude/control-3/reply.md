Spec read. Here's the implementation — a recursive-descent parser over the raw string so error positions are exact.
...
Notes on the error cases from the spec: each one lands where required — `2 +` and `(1 + 2` fail at the input length (position 3 and 6) because the failure is at end of input, `1 + )` at 4 and `1 $ 2` at 2 on the offending character, and empty input at 0. `right === 0` also catches `-0`, so `1 / -0` raises `RangeError` rather than returning `-Infinity`.

I did not write the file, per your instruction — the code block above is the complete contents of `evaluate.ts`.
