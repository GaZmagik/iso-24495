I read the spec and implemented a recursive-descent parser. Here is `evaluate.ts`:
...
Design notes on the two parts the spec pins down:

- **Precedence of `^` against a sign.** `parsePower` reads its exponent through `parseUnary`, so `2^3^2` recurses right and gives 512, and `-2^2` negates the whole power and gives -4.
- **Error positions.** `peek` skips whitespace before reporting, so the recorded index lands on the offending character rather than a preceding space. `fail` substitutes the input length when the input has run out.

Status: written and hand-traced against all five error examples in the spec, plus `2^3^2` and `-2^2`. I ran no compiler and no tests, because you asked for output only, so type-checking and execution are still open.
